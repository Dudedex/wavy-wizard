...
function waveBreathSfx(color) {
  ...
  noise(0.22, color === 'ice' ? 'highpass' : 'bandpass', low, high, color === 'earth' ? 0.055 : 0.045);
  noise(0.42, color === 'wind' ? 'bandpass' : 'lowpass', high, low, color === 'fire' ? 0.075 : 0.06, 0.08);
  blip(color === 'earth' ? 120 : 260, color === 'earth' ? 55 : 140, 0.22, 'triangle', 0.026, 0.11);
}
...
function spellSfx(id) {
  ...
  switch (id) {
    ...
    case 'firebreath': // flame wave: swell, crest, and hot crash
      waveBreathSfx('fire');
      blip(180, 95, 0.18, 'sawtooth', 0.026, 0.18);
      break;
    case 'icebreath': // freezing wave: glassy swell with icy spray
      waveBreathSfx('ice');
      chord([1400, 2100, 0.16, 'sine', 0.014, 0.07], [2100, 2800, 0.14, 'sine', 0.012, 0.16]);
      break;
    case 'earthbreath': // ground wave: rolling rumble and impact
      waveBreathSfx('earth');
      blip(90, 45, 0.30, 'sawtooth', 0.052, 0.16);
      break;
    case 'windbreath': // air wave: rushing swell and trailing gust
      waveBreathSfx('wind');
      blip(620, 980, 0.18, 'sine', 0.014, 0.15);
      break;
    default:
